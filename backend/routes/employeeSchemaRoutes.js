...
const EmployeeModelService = require("../models/employeeSchema");
...
router.post("/employeeModal", validateEmployeeMiddleware, asyncHandler(async (req, res) => {
...
router.get("/employeeModal", authMiddleware, asyncHandler(async (req, res) => {
    const employee = await EmployeeModelService.find();
    console.log(employee);
    res.status(200).json(employee);
}));
// BULK CREATE Employee
router.post("/employeeModal/bulk", asyncHandler(async (req, res) => {
...
router.put("/employeeModal/:id", asyncHandler(async (req, res) => {
    const updatedEmployee = await EmployeeModelService.findOneAndUpdate({ employeeId: String(req.params.id) }, req.body, { new: true });
    if (!updatedEmployee) {
        res.status(404).json({
            message: "Employee Not Found",
        });
...
//# sourceMappingURL=employeeSchemaRoutes.js.map